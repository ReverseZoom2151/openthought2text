...
def test_audit_detects_text_group_and_interval_leakage():
    ...
    assert {"DUPLICATE_TARGET_TEXT", "GROUP_ACROSS_SPLITS", "CONTINUOUS_INTERVAL_OVERLAP"} <= codes(report)
...
def test_audit_detects_declared_pretraining_overlap_and_inference_text():
    row = sample()
    report = audit_splits(
        (row,), information_access=InformationAccess(inference_target_text=True),
...
def test_audit_labels_aligned_access_and_rejects_audio_access():
    ...
    assert {"INFERENCE_TOKEN_BOUNDARIES", "INFERENCE_EVENT_BOUNDARIES", "INFERENCE_STIMULUS_AUDIO"} <= codes(report)
    assert not report.passed
