...
from openthought2text.models import ContinuousNeuralEncoder, NeuralToTextModel, TargetFreeAutoregressiveDecoder
...
def test_supervised_step_updates_composed_model() -> None:
    ...
    decoder = TargetFreeAutoregressiveDecoder(vocab_size=12, hidden_size=16, num_layers=1, num_heads=4)
    ...
    result = supervised_train_step(model, batch, torch.tensor([[1, 2, 3], [2, 3, 4]]), optimizer=optimizer)
