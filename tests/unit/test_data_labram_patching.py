...
from openthought2text.data import LaBraMInputPatchConfig, TensorBackedSample, patch_tensor_backed_sample
...
def test_patch_contract_segments_deterministically_and_zeroes_padding() -> None:
    ...
    assert patched.patches[2].tolist() == [[0., 0.], [0., 0.]]
    assert patched.patches[:, 1, :].tolist() == [[0., 0.], [0., 0.], [0., 0.]]
...
def test_patch_contract_is_label_invariant_and_validates_sample_rate() -> None:
    ...
    assert torch.equal(patch_tensor_backed_sample(row(), config).patches, patch_tensor_backed_sample(changed, config).patches)
