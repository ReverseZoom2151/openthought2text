...
def row(*, safe_shift=False):
    ...
    return TensorBackedSample(
        sample=source,
        values=torch.tensor([[1., 2., 99.], [3., 4., 99.]]),
...
def test_augmentation_is_seeded_label_invariant_and_never_changes_masked_values() -> None:
    config = NeuralAugmentationConfig(temporal_mask_probability=.4, channel_dropout_probability=.4, additive_noise_std=.2)
    ...
    assert first.values[:, 2].tolist() == [0., 0.]
...
def test_batch_augmentation_preserves_padding_zeroes_and_masks() -> None:
    batch = collate_tensor_backed_samples((row(),))
    augmented = augment_neural_tensor_batch(batch, NeuralAugmentationConfig(additive_noise_std=.5), seed=3)
    assert augmented.signals[0, :, 2].tolist() == [0., 0.]
