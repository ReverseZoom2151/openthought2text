...
def _spec():
    ...
    return TargetFreeEvaluationSpec(_hash("a"), bind("model", "b"), bind("checkpoint", "c"), bind("config", "d"), tuple(ControlCondition), (BenchmarkRowLabel("zuco", "eeg", "read", "trial", "loso", "open", "greedy"),), ("neural_signal", "sample_mask"), ("predictions.jsonl", "evaluation.json"))
...
def test_execution_spec_rejects_target_input_and_missing_full_control():
    with pytest.raises(ProvenanceError, match="cannot declare"):
        TargetFreeEvaluationSpec(_hash("a"), _spec().model, _spec().checkpoint, _spec().resolved_config, tuple(ControlCondition), _spec().benchmark_rows, ("target_text",), ("x",))
    with pytest.raises(ProvenanceError, match="include full"):
        TargetFreeEvaluationSpec(_hash("a"), _spec().model, _spec().checkpoint, _spec().resolved_config, (ControlCondition.ZERO,), _spec().benchmark_rows, ("neural",), ("x",))
