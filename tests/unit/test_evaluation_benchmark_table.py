...
def _row(dataset: str, run: str) -> BenchmarkTableRow:
    return BenchmarkTableRow(
        BenchmarkRowLabel(dataset, "eeg", "reading", "trial", "loso", "open", "greedy"),
        run,
        {"wer": 0.2, "retrieval_mrr": 0.7},
        {"wer": MetricUncertainty(0.1, 0.3, 0.95)},
        BenchmarkProvenanceReferences(
            "evaluation.json", "provenance.json", _digest("a"), _digest("b")
        ),
    )
...
def test_row_rejects_unknown_uncertainty_and_malformed_provenance_digest() -> None:
    with pytest.raises(ValueError, match="metrics not present"):
        BenchmarkTableRow(
            ...
            "run",
            {"wer": 0.2},
            {"cer": MetricUncertainty(0.1, 0.3, 0.95)},
            BenchmarkProvenanceReferences("e", "p", _digest("a"), _digest("b")),
        )
