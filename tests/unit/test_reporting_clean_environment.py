from openthought2text.reporting import CleanEnvironmentAuditPlan, audit_clean_environment, render_clean_environment_markdown
...
def test_clean_environment_audit_binds_plan_and_caller_observations_without_run_claim():
    plan = CleanEnvironmentAuditPlan("Python 3.12", "openthought2text==0.0.1", _hash("a"), "python -m pytest", {"config.json": _hash("b"), "model.pt": _hash("c")})
    record = audit_clean_environment(plan, {"python": "3.12.3", "platform": "linux"}, {"config.json": _hash("b"), "model.pt": _hash("d")})
