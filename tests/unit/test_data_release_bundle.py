...
def release_inputs(root):
    ...
    plan_path.write_text(
        json.dumps(
            build_split_plan(manifest.samples, SplitProtocol.RANDOM_LEGACY, seed=2).to_dict()
        ),
        encoding="utf-8",
    )
...
def test_release_bundle_binds_validated_artifacts_and_information_contract(tmp_path) -> None:
    ...
    bundle = build_dataset_release_bundle(
        tmp_path,
        dataset_card=card,
        canonical_manifest=manifest,
        derived_split_plan=plan,
...
def test_release_bundle_audit_detects_changed_bound_artifact(tmp_path) -> None:
    ...
    assert "ARTIFACT_CHECKSUM_MISMATCH" in {
        item.code for item in audit_dataset_release_bundle(bundle_path).issues
    }
