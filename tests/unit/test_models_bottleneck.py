...
def test_bottleneck_ignores_arbitrary_padded_values_in_continuous_and_vq_paths():
    ...
    bottleneck = NeuralRepresentationBottleneck(
        3,
        ResidualVectorQuantizer(ResidualVectorQuantizerConfig(num_codebooks=2, codebook_size=4, embedding_dim=3)),
    ).eval()
    ...
    torch.testing.assert_close(first.continuous_features, second.continuous_features, rtol=0, atol=0)
