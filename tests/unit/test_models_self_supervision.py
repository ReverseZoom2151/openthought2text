...
def test_self_supervision_reconstructs_and_matches_only_valid_tokens_with_gradients():
    ...
    output = objective(primary, primary_mask, targets, reconstruction_mask, secondary, secondary_mask)
    assert output.reconstruction.shape == (2, 4, 3)
    assert output.reconstruction_token_count.item() == 3
    assert output.consistency_mask.tolist() == [[True, False, False, False], [True, False, True, False]]
...
def test_self_supervision_ignores_padded_target_and_secondary_values_deterministically():
    ...
    first = objective(primary, primary_mask, targets, secondary_features=secondary, secondary_mask=secondary_mask)
    changed_targets = targets.clone()
    changed_targets[:, 2] = 1_000
    changed_secondary = secondary.clone()
    changed_secondary[:, 1:] = -1_000
    second = objective(primary, primary_mask, changed_targets, secondary_features=changed_secondary, secondary_mask=secondary_mask)
    torch.testing.assert_close(first.loss, second.loss, rtol=0, atol=0)
    torch.testing.assert_close(first.reconstruction_loss, second.reconstruction_loss, rtol=0, atol=0)
...
def test_self_supervision_validates_masks_and_optional_view_pairs():
    ...
    with pytest.raises(ValueError, match="match primary"):
        objective(features, mask, targets, secondary_features=torch.randn(1, 3, 4), secondary_mask=torch.ones(1, 2, dtype=torch.bool))
