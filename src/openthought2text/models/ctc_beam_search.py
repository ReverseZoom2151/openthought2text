...
class CTCBeamSearchConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            isinstance(self.blank_token_id, bool)
            or not isinstance(self.blank_token_id, int)
            or self.blank_token_id < 0
        ):
...
class ValidationFittedLanguageScorer:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            isinstance(self.validation_examples, bool)
            or not isinstance(self.validation_examples, int)
            or self.validation_examples < 1
        ):
...
class TargetFreeCTCBeamSearch:
    ...
    def _rank(
        ...
        blank: float,
        nonblank: float,
        scorer: ValidationFittedLanguageScorer | None,
    ...
    def _decode_one(
        self, log_probs: torch.Tensor, scorer: ValidationFittedLanguageScorer | None
    ) -> tuple[CTCBeamHypothesis, ...]:
            ...
            for prefix, (prob_blank, prob_nonblank) in beams.items():
                for token, token_log_prob in enumerate(timestep):
                    if token == self.config.blank_token_id:
                        ...
                        next_beams[prefix] = (
                            _log_add(
                                old_blank, _log_add(prob_blank, prob_nonblank) + token_log_prob
                            ),
                            old_nonblank,
                        )
                        ...
                        next_beams[prefix] = (
                            old_blank,
                            _log_add(old_nonblank, prob_nonblank + token_log_prob),
                        )
                        extended = prefix + (token,)
                        old_blank, old_nonblank = next_beams.get(extended, (-math.inf, -math.inf))
                        next_beams[extended] = (
                            old_blank,
                            _log_add(old_nonblank, prob_blank + token_log_prob),
                        )
                    else:
                        extended = prefix + (token,)
                        old_blank, old_nonblank = next_beams.get(extended, (-math.inf, -math.inf))
                        next_beams[extended] = (
                            old_blank,
                            _log_add(
                                old_nonblank, _log_add(prob_blank, prob_nonblank) + token_log_prob
                            ),
                        )
            ranked = sorted(
                next_beams.items(),
    ...
    def decode(
        ...
        if valid_lengths.shape != (batch,) or valid_lengths.dtype not in (
            torch.int8,
            torch.int16,
            torch.int32,
            torch.int64,
        ...
        if language_scorer is not None and not isinstance(
            language_scorer, ValidationFittedLanguageScorer
        ):
        ...
        rows = [
            self._decode_one(log_probs[index, :length].cpu(), language_scorer)
            for index, length in enumerate(lengths.tolist())
        ]
        widest = max(len(row) for row in rows)
        neural_scores = torch.full(
            (batch, widest), -torch.inf, dtype=log_probs.dtype, device=log_probs.device
        )
        combined_scores = torch.full_like(neural_scores, -torch.inf)
        language_scores = (
            None if language_scorer is None else torch.full_like(neural_scores, -torch.inf)
        )
        ...
        return CTCBeamSearchOutput(
            tuple(rows), neural_scores, language_scores, combined_scores, hypothesis_mask
        )
