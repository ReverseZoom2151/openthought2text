...
class OrderedSemanticAnchorDecoderConfig:
    ...
    def __post_init__(self):
        ...
        if not 0 <= self.pad_anchor_id < self.num_anchors:
            raise ValueError("pad_anchor_id must belong to the anchor vocabulary")
        if self.eos_anchor_id is not None and not 0 <= self.eos_anchor_id < self.num_anchors:
            raise ValueError("eos_anchor_id must belong to the anchor vocabulary")
...
class OrderedSemanticAnchorGenerationConfig:
    ...
    def __post_init__(self):
        if self.max_new_anchors < 1:
            raise ValueError("max_new_anchors must be positive")
...
class OrderedSemanticAnchorDecoder(nn.Module):
    ...
    def __init__(self, config: OrderedSemanticAnchorDecoderConfig):
        super().__init__()
        self.config = config
        self.start_token_id = config.num_anchors
        ...
        self.decoder, self.norm, self.output = (
            nn.TransformerDecoder(layer, config.num_layers),
            nn.LayerNorm(config.hidden_size),
            nn.Linear(config.hidden_size, config.num_anchors, bias=False),
        )

    def _memory(self, features, mask):
        if features.ndim != 3 or features.shape[-1] != self.config.hidden_size:
            raise ValueError("neural_features must be [batch, tokens, hidden_size]")
        if mask.shape != features.shape[:2] or not mask.bool().any(dim=1).all():
            raise ValueError("neural_mask must match features and retain one token per example")

    def _decode(self, ids, features, mask):
        batch, length = ids.shape
        if length > self.config.max_sequence_length:
            raise ValueError("anchor sequence exceeds max_sequence_length")
        ...
        decoded = self.decoder(
            ...
            features,
            tgt_mask=causal,
            tgt_key_padding_mask=ids.eq(self.config.pad_anchor_id),
    ...
    def training_forward(self, neural_features, neural_mask, anchor_targets, target_mask=None):
        ...
        if (
            anchor_targets.ndim != 2
            or anchor_targets.shape[0] != neural_features.shape[0]
            or anchor_targets.shape[1] > self.config.max_sequence_length
        ):
            raise ValueError("anchor_targets must be bounded [batch, anchors]")
        if target_mask is None:
            target_mask = anchor_targets.ne(self.config.pad_anchor_id)
        if target_mask.shape != anchor_targets.shape:
            raise ValueError("target_mask must match anchor_targets")
        valid = target_mask.bool()
        if (anchor_targets[valid] < 0).any() or (
            anchor_targets[valid] >= self.config.num_anchors
        ).any():
            raise ValueError("valid anchor_targets must belong to the anchor vocabulary")
        ...
        loss = (
            F.cross_entropy(logits.flatten(0, 1), labels.flatten(), ignore_index=-100)
            if valid.any()
            else logits.sum() * 0.0
        )
    ...
    def generate(
        ...
        neural_features,
        neural_mask,
        config: OrderedSemanticAnchorGenerationConfig | None = None,
    ):
        self._memory(neural_features, neural_mask)
        config = config or OrderedSemanticAnchorGenerationConfig()
        if config.max_new_anchors > self.config.max_sequence_length:
            raise ValueError("max_new_anchors exceeds max_sequence_length")
        eos = self.config.eos_anchor_id if config.eos_anchor_id is None else config.eos_anchor_id
        if eos is not None and not 0 <= eos < self.config.num_anchors:
            raise ValueError("generation eos_anchor_id must belong to the anchor vocabulary")
        batch = neural_features.shape[0]
        inputs = torch.full(
            (batch, 1), self.start_token_id, dtype=torch.long, device=neural_features.device
        )
        finished = torch.zeros(batch, dtype=torch.bool, device=neural_features.device)
        rows = []
        masks = []
        for _ in range(config.max_new_anchors):
            next_ids = self._decode(inputs, neural_features, neural_mask)[:, -1].argmax(dim=-1)
            active = ~finished
            rows.append(
                torch.where(active, next_ids, torch.full_like(next_ids, self.config.pad_anchor_id))
            )
            masks.append(active)
            if eos is not None:
                finished |= next_ids.eq(eos)
