...
class FrequencyTextBaseline:
    ...
    def fit(self, training_texts: Sequence[str]) -> FrequencyTextBaseline:
        if not training_texts or any(
            not isinstance(text, str) or not text.strip() for text in training_texts
        ):
            raise ValueError("training_texts must be non-empty text strings")
        counts = Counter(training_texts)
        self._prediction = min(
            (text for text, count in counts.items() if count == max(counts.values()))
        )
        return self
...
class NearestNeighborTextRetrieval:
    ...
    def fit(
        self, training_neural_embeddings: Sequence[Sequence[float]], training_texts: Sequence[str]
    ) -> NearestNeighborTextRetrieval:
        embeddings = _matrix(training_neural_embeddings, "training_neural_embeddings")
        if len(embeddings) != len(training_texts) or any(
            not text.strip() for text in training_texts
        ):
    ...
    def predict(self, neural_embeddings: Sequence[Sequence[float]]) -> tuple[str, ...]:
        ...
        for query in queries:
            best = max(
                ...
                key=lambda index: (_cosine(query, self._embeddings[index]), -index),
            )
    ...
    def artifact(self) -> BaselineArtifact:
        ...
        return BaselineArtifact.create(
            "nearest_neighbor_text",
            {"embeddings": [list(row) for row in self._embeddings], "texts": list(self._texts)},
        )
...
class RidgeRegressor:
    ...
    def fit(
        ...
        training_targets: Sequence[float] | Sequence[Sequence[float]],
    ) -> RidgeRegressor:
        ...
        gram = [
            [
                ...
                + (self.alpha if i == j and (not self.fit_intercept or i != 0) else 0.0)
        ...
        rhs = [
            [
                sum(design[row][i] * targets[row][output] for row in range(len(design)))
    ...
    def predict(
        self, neural_features: Sequence[Sequence[float]]
    ) -> tuple[float, ...] | tuple[tuple[float, ...], ...]:
        if self._weights is None:
            raise RuntimeError("ridge regressor must be fit before inference")
        features = _matrix(
            neural_features, "neural_features", width=len(self._weights) - int(self.fit_intercept)
        )
        outputs = []
        for row in features:
            ...
            outputs.append(
                tuple(
                    sum(design[i] * self._weights[i][j] for i in range(len(design)))
...
def _matrix(
    ...
    if expected <= 0 or any(
        len(row) != expected or not all(math.isfinite(item) for item in row) for row in rows
    ):
...
def _target_matrix(
    values: Sequence[float] | Sequence[Sequence[float]], rows: int
) -> list[tuple[float, ...]]:
    if len(values) != rows:
        raise ValueError("training targets must align with training features")
    matrix = [
        (float(item),) if isinstance(item, (int, float)) else tuple(float(value) for value in item)
        for item in values
    ]
    if not matrix or any(
        len(row) != len(matrix[0]) or not all(math.isfinite(item) for item in row) for row in matrix
    ):
