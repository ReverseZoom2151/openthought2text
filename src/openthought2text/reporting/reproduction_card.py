...
class SourceReference:
    ...
    def __post_init__(self) -> None:
        if any(not isinstance(getattr(self, field), str) or not getattr(self, field).strip() for field in self.__dataclass_fields__):
            raise ProvenanceError("source references require explicit citation, URL, and version/revision")
...
class ReproductionProvenanceCard:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.allowed_inference_inputs or any(not item.strip() for item in self.allowed_inference_inputs):
            raise ProvenanceError("allowed_inference_inputs must be explicit and non-empty")
        if any(any(forbidden in item.casefold() for forbidden in _FORBIDDEN_INFERENCE) for item in self.allowed_inference_inputs):
            raise ProvenanceError("allowed_inference_inputs cannot include text labels, targets, or references")
    ...
    def binding_dict(self) -> dict[str, Any]:
        return {
            ...
            "source_paper": self.source_paper.to_dict(), "source_repository": self.source_repository.to_dict(),
            "fidelity_summary": self.fidelity_summary, "deviations": list(self.deviations),
            "allowed_inference_inputs": list(self.allowed_inference_inputs),
            "split_plan": self.split_plan.to_dict(), "resolved_config": self.resolved_config.to_dict(),
            "checkpoint": self.checkpoint.to_dict(), "architecture_sha256": self.architecture_sha256,
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "ReproductionProvenanceCard":
        ...
        card = cls(
            baseline_name=str(data["baseline_name"]), source_paper=SourceReference.from_dict(data["source_paper"]),
            source_repository=SourceReference.from_dict(data["source_repository"]), fidelity_summary=str(data["fidelity_summary"]),
            deviations=tuple(data["deviations"]), allowed_inference_inputs=tuple(data["allowed_inference_inputs"]),
            split_plan=ArtifactBinding.from_dict(data["split_plan"]), resolved_config=ArtifactBinding.from_dict(data["resolved_config"]),
            checkpoint=ArtifactBinding.from_dict(data["checkpoint"]), architecture_sha256=str(data["architecture_sha256"]),
            performance_claims=str(data["performance_claims"]), schema_version=str(data["schema_version"]),
        )
...
def write_reproduction_card(path: str | Path, card: ReproductionProvenanceCard) -> None:
    ...
    destination.write_text(json.dumps(card.to_dict(), indent=2, sort_keys=True) + "\n", encoding="utf-8")


def read_reproduction_card(path: str | Path) -> ReproductionProvenanceCard:
    try:
        return ReproductionProvenanceCard.from_dict(json.loads(Path(path).read_text(encoding="utf-8")))
