...
class FailureCase:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "control_predictions": dict(sorted(self.control_predictions.items())),
        }
...
class FailureCaseExplorerArtifact:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not self.prediction_artifact.strip()
            or not self.evaluation_artifact.strip()
        ):
            raise ProvenanceError("failure explorer requires explicit run and artifact references")
        if len({case.sample_id for case in self.cases}) != len(self.cases):
            raise ProvenanceError("failure explorer sample IDs must be unique")
        if (
            self.no_performance_claim
            != "Failure-review artifact only; no performance claim is made."
        ):
            raise ProvenanceError("failure explorer cannot contain performance claims")

    @property
    def binding_sha256(self) -> str:
        return _digest(self.binding_dict())
    ...
    def binding_dict(self) -> dict[str, Any]:
        return {
            ...
            "no_performance_claim": self.no_performance_claim,
        }
...
def build_failure_case_explorer(
    ...
    errors: Sequence[TextErrorRecord],
    ...
    *,
    ...
    if not records or not errors:
        raise ValueError("saved prediction and error records are required")
    run_ids = {record.run_id for record in records}
    if len(run_ids) != 1:
        raise ValueError("prediction records must have one run_id")
    by_control: dict[str, dict[str, PredictionRecord]] = {}
    for record in records:
        by_control.setdefault(record.control.value, {})[record.sample_id] = record
    if ControlCondition.FULL.value not in by_control:
        raise ValueError("failure explorer requires full predictions")
    error_by_id = {error.sample_id: error for error in errors}
    full_ids = set(by_control[ControlCondition.FULL.value])
    if full_ids != set(error_by_id):
        raise ValueError("error records must match full prediction sample IDs")
    for control, rows in by_control.items():
        if set(rows) != full_ids:
            raise ValueError(f"{control} predictions are not paired to full samples")
    declared = {item.condition.value for item in control_results}
    if not set(by_control).issubset(declared):
        raise ValueError("every saved prediction control must appear in control results")
    cases = tuple(
        FailureCase(
            sample_id,
            error_by_id[sample_id].reference,
            by_control["full"][sample_id].prediction_text,
            error_by_id[sample_id].category.value,
            {
                control: rows[sample_id].prediction_text
                for control, rows in by_control.items()
                if control != "full"
            },
        )
        for sample_id in sorted(full_ids)
    )
    return FailureCaseExplorerArtifact(
        next(iter(run_ids)),
        prediction_artifact,
        evaluation_artifact,
...
def render_failure_case_explorer_markdown(artifact: FailureCaseExplorerArtifact) -> str:
    lines = [
        ...
        "",
        f"**{artifact.no_performance_claim}**",
        "",
        f"Run: `{artifact.run_id}`  ",
        f"Binding: `{artifact.binding_sha256}`",
        "",
        "| Sample | Category | Reference | Full prediction | Control predictions |",
        "| --- | --- | --- | --- | --- |",
    ]
    for case in artifact.cases:
        controls = (
            "; ".join(f"{name}={value}" for name, value in sorted(case.control_predictions.items()))
            or "—"
        )
        lines.append(
            f"| {case.sample_id} | {case.error_category} | {case.reference} | {case.full_prediction} | {controls} |"
        )
    return "\n".join(lines) + "\n"
...
def validate_complete_control_suite_plan(
    spec: TargetFreeEvaluationSpec,
    *,
    required_controls: Sequence[ControlCondition] = tuple(ControlCondition),
    required_outputs: Sequence[str] = ("predictions.jsonl", "evaluation.json", "provenance.json"),
    ...
    missing_controls = tuple(
        sorted(item.value for item in set(required_controls).difference(spec.control_conditions))
    )
    declared_outputs = set(spec.required_output_artifacts)
    missing_outputs = tuple(
        sorted(item for item in required_outputs if item not in declared_outputs)
    )
    return ControlSuitePlanValidation(
        spec.binding_sha256,
        missing_controls,
        missing_outputs,
        not missing_controls and not missing_outputs,
    )
